...
This roadmap delivered a working MCP server in Rust that serves curated sources via HTTP POST. Built local-first (no Pubky SDK dependency), from data layer up through protocol implementation to deployment. The 7-phase structure followed natural dependency boundaries: foundation, business logic (matching), protocol layer (MCP), HTTP transport, identity (PKARR), infrastructure (Docker/Render), and final documentation/testing. Every requirement mapped to exactly one phase, ensuring complete coverage while maintaining independent verification at each stage.
...
**Key Decisions:**
- Local-first architecture: Build without Pubky SDK dependency, PKARR keypair only for identity
- HTTP POST only for MCP transport (no stdio, no SSE for MVP)
- Permissive CORS for MVP simplicity
- debian:bookworm-slim over Alpine for compatibility (133MB image)
- pkarr with curve25519-dalek git patch for compilation
- Weighted sum (not multiplicative) for fuzzy + keyword score combination
- Plain text tool responses for better agent parsing
- Mermaid for architecture diagrams in README

**Issues Resolved:**
- pkarr 5.0 dependency required curve25519-dalek git patch (pre-release issue)
- Edition 2024 required Rust 1.85+ (not 1.84 as originally planned)
- Docker image 133MB vs planned <50MB (debian chosen for compatibility)

**Issues Deferred:**
- Render service creation and DNS for api.3gs.ai (human setup)
- PUBKY_HOMESERVER env var (v1 doesn't publish to homeserver)

**Technical Debt Incurred:**
- curve25519-dalek git patch in Cargo.toml (fragile)
- McpError enum defined but unused (ToolCallError used instead)
- RegistryError::DuplicateSlug variant never constructed
- MatchResult.score field populated but not exposed to consumers
- Keypair not stored in AppState (only PublicKey)
